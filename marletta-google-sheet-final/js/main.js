...
async function loadData() {
  ...
  try {
    const [categories, products, attributes] = await Promise.all([
      fetchCsv(urls.categoriesUrl),
      fetchCsv(urls.productsUrl),
      fetchCsv(urls.attributesUrl),
    ]);
    ...
    console.error(error);
    els.sourceBadge.textContent = "Източник: грешка при зареждане";
    throw error;
  }
}
...
async function bootstrap() {
  ...
  try {
    ...
    els.productGrid.innerHTML = `<div class="load-error">Не успях да заредя каталога. Провери js/config.js и дали Google Sheets файлът е достъпен за четене.</div>`;
